...
impl Muxer {
    ...
    /// Write a pre-encoded Opus packet (passthrough) to the audio track.
    /// `pts` is ignored on the Rust side: the C++ muxer stamps the packet with
    /// the same wall-clock used for video (`self.start.elapsed()`), so audio and
    /// video share one container timeline.
    pub fn write_audio(&mut self, data: &[u8], _pts: i64) -> Result<(), i32> {
        unsafe {
            let result = hwcodec_write_audio_frame(
                self.inner,
                (*data).as_ptr(),
                data.len() as _,
                self.start.elapsed().as_millis() as _,
            ...
            if result != 0 {
                if av_log_get_level() >= AV_LOG_ERROR as _ {
                    error!("Error write_audio: {}", result);
                }
                return Err(result);
            }
            Ok(())
